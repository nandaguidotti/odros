...
out.println("function initMap() {");
 ...
 for (int i = 0; i < listaLat.size();i++){
   ...
   //  out.println("pontosLat["+i+"]= lat: " + listaLat.get(i)+ ";");   // Usar com a poliLyne
   //  out.println("pontosLong["+i+"]= lng: " + listaLong.get(i)+ ";"); // Usar com a poliLyne 
 
    //    out.println("['First Shoppe'," + listaLat.get(i) + "," + listaLong.get(i)+"]");  // tem um virgula aqui no final que foi tirada para teste
       // out.println("['Second Shoppe', -220.059614, -478.986926]");
       // out.println("['Third Shoppe', -37.816935, 144.966877],");
       // out.println("['Fourth Shoppe', -37.818714, 145.036494],");
       // out.println("['Fifth Shoppe', -37.793834, 144.987018]");			  
 } 
   ...
   out.println("];");
    ...
    out.println("<script async defer "
            + "src=\"https://maps.googleapis.com/maps/api/js?key=AIzaSyAuFq5j0XisThfVc34csR7AyBndr_vEEl0&callback=initMap\">");               
    out.println("</script>");
